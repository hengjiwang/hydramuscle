...
class Shell:
    ...
    def init_stimulation_pattern(self, behavior):
        if behavior == 'contraction burst':
            ...
            self.s_ip = random.sample(self.s_ip, 1000)
...
if __name__ == "__main__":
    model = Shell(SMC(T=100, dt=0.0002, k2=0.2, s0=200, d=40e-4, v7=0.02), 
    'elongation', numx=100, numy=100)
    sol = model.run([-100], [10])
    df = pd.DataFrame(sol[:,0:model.numx*model.numy])
    df.to_csv('/media/hengji/DATA/Data/Documents/hydramuscle/results/data/calcium/100x100_100s_elongation_k2_02_s0_200_v7_002_gip3_10.csv', index = False)