...
class Shell:
    ...
    def __init__(self, cell, numx, numy, save_interval):
        ...
        self.init_stimulation_pattern('slow', 0, -1, 0, -1, int(0.1*self.num2)) # Active Elongation Stimulation
    ...
    def rhs(self, y, t, stims_fast, stims_slow):
        ...
        dcdt = i_ipr + i_leak - i_serca + i_in - i_pmca - self.cell.alpha * (i_cal + i_cat) + self.cell.fluo_ratio * (-ir1 - ir2 - ir3 - ir4)
    ...
    def run(self, stims_fast = [1,7,10,13,16,19,22,25,29,34,40,50], stims_slow = [35]):
        # Time stepping
...
if __name__ == "__main__":
    model = Shell(SMC(T=200, 
                      ...
                      d=float(sys.argv[3]), 
                      v7=float(sys.argv[4]), 
                      active_v8=float(sys.argv[5]),
                      fluo_ratio=1,
                      k9=float(sys.argv[8]),
                      gkca=float(sys.argv[9])), 
                  numx=int(sys.argv[6]), 
                  numy=int(sys.argv[7]), 
                  save_interval=100)
    ...
    df.to_csv('/media/hengji/DATA/Data/Documents/hydramuscle/results/data/calcium/'+
              ...
              str(model.T) + 's_cycle_endo_' + 
              'k2_' + str(model.cell.k2).replace('.', '') +
              '_s0_' + str(model.cell.s0) + 
              '_v7_' + str(model.cell.v7).replace('.', '') + 
              '_activev8_' + str(model.cell.active_v8).replace('.', '') + 
              '_k9_' + str(model.cell.k9).replace('.', '') + 
              '_gkca_' + str(model.cell.gkca).replace('.', '') + 
              '.csv', index = False)